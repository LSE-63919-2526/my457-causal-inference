...
The coding exercise this week will walk through a few general principles of the potential outcomes framework and how assignment mechanisms shape our ability to learn from observed data. First, we will load in some required packages and set a seed for reproducibility.
...
package_check(c("dplyr", "ggplot2", "ggpubr", "ggdag", "estimatr", "Hmisc"))
...
data = tibble(
  X = rnorm(N), # from a normal distribution
  U = rnorm(N),
...
# Regression:
lm(Y ~ D, data_selection) # classical SEs
lm_robust(Y ~ D, data_selection) # easy way to estimate robust SEs (default = HC2)
```
...
# Regression:
lm_robust(Y ~ D, data_simple)
lm_robust(Y ~ D, data_complete)
```
...
```{r experiment_viz}
# fit the model you wish to plot:
lm_complete_nocov <- lm_robust(Y ~ D, data_complete)
...
data_complete %>%
  ...
  theme_minimal() +
  scale_x_continuous(limits = c(-0.5,1.5), breaks = c(0,1), labels = c("Control", "Treated")) +
  # add text for point estimates:
  geom_text(aes(label = round(after_stat(y), 2)), stat = "summary", vjust = -1) +
  geom_bracket(xmin = 0, xmax = 1, y.position = 2.5, label = paste0("ATE = ", round(lm_complete_nocov$coefficients["D"], 3), " (p = ", round(lm_complete_nocov$p.value["D"], 3), ")")) +
  labs(title = "Mean Values of Y by Treatment Condition", x = "Experimental Group", y = "Y")
```
